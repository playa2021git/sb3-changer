...
- ImageClassifier2Scratch、ML2Scratch、Posenet2Scratch、TM2Scratch、TMPose2Scratch、Speech2Scratch、Camera Selector の未確認ブロック
- Microbit Moreのうち、`TILT_LEFT` / `TILT_RIGHT`、サーボ、ピン入出力、センサー値取得など未検証のブロック
- Scratchの独自ブロック定義
...
| Microbit More | 確認済み最小範囲のみ対応。回帰テストで保護 | A/Bボタン、テキスト表示、シェイク、音再生/停止 |

## Microbit Moreの現在の対応範囲

Microbit Moreは、授業で安全に使えることを優先し、保存形と動作を確認できた最小範囲だけを変換対象にしています。
公式 `getInfo()` 上の他ブロックも棚卸し済みですが、保存形と実機動作が揃うまでは個別名で安全停止します。
...
詳細は [Microbit More対応表](docs/microbit-more-block-matrix.md)、[fixture採取手順](docs/microbit-more-fixture-guide.md)、[実装計画](docs/microbit-more-implementation-plan.md) を参照してください。
