...
test("Microbit More: 明るさ・roll reporterを公式fixtureの保存形で変換できる", () => {
  ...
  const ledger = JSON.parse(readFileSync(path.join(rootDir, "definitions/microbit-more.json"), "utf8"));
  const rollDefinition = ledger.blocks.find((block) => block.functionName === "microbitRoll");
  assert.equal(rollDefinition.supportStatus, "stable");
  assert.equal(rollDefinition.deviceVerificationStatus, "passed");
});

test("Microbit More: pull modeとP0デジタル出力を公式fixtureの保存形で変換できる", () => {
  const result = convert(`whenGreenFlagClicked(() => {
  ...
  const fixturePath = "fixtures/microbit-more/official-hc-sr04.sb3";
  assertBlockMatchesOfficialFixture(result, fixturePath, {
    opcode: "microbitMore_setPullMode",
    fields: { PIN: "1", MODE: "NONE" }
  });
  assertBlockMatchesOfficialFixture(result, fixturePath, {
  ...
  const officialProject = readProjectJsonFromSb3(path.join(rootDir, fixturePath));
  const officialDigitalBlocks = findFixtureBlocks(officialProject, {
    opcode: "microbitMore_setDigitalOut",
    fields: { PIN: "0" }
  });
  const generatedDigitalBlocks = findFixtureBlocks(result.project, {
    opcode: "microbitMore_setDigitalOut",
    fields: { PIN: "0" }
  });
  assert.deepEqual(
    normalizeBlockShape(generatedDigitalBlocks[1].target, generatedDigitalBlocks[1].blockId),
    normalizeBlockShape(officialDigitalBlocks[2].target, officialDigitalBlocks[2].blockId)
  );

  const blocks = Object.values(spriteBlocks(result));
  const levelShadows = blocks
    .filter((block) => block.opcode === "microbitMore_menu_digitalValueMenu")
    .map((block) => block.fields.digitalValueMenu?.[0])
    .sort();
  assert.deepEqual(levelShadows, ["false", "true"]);
});

test("Microbit More: fixtureにないpin・pull mode・digital値は安全停止する", () => {
  const pullPinError = conversionError(`whenGreenFlagClicked(() => {
  microbitSetPullMode("0", "NONE");
});`);
  assert.match(pullPinError.message, /未確認のメニュー値/);

  const pullModeError = conversionError(`whenGreenFlagClicked(() => {
  microbitSetPullMode("1", "UP");
});`);
  assert.match(pullModeError.message, /未確認のメニュー値/);

  const digitalPinError = conversionError(`whenGreenFlagClicked(() => {
  microbitSetDigitalOut("1", "true");
});`);
  assert.match(digitalPinError.message, /未確認のメニュー値/);

  const digitalLevelError = conversionError(`whenGreenFlagClicked(() => {
  microbitSetDigitalOut("0", "HIGH");
});`);
  assert.match(digitalLevelError.message, /未確認のメニュー値/);
});
